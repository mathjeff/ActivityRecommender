...
// The Activity subclass represents a category of things to do, for example Fun, Exercise, or Work

// The ToDo subclass represents a specific task that has a completion status and has no child activities, for example 
namespace ActivityRecommendation
{
    public abstract class Doable : INumerifier<WillingnessSummary>
    {
        #region Constructors
        ...
        // public
        public Doable(RatingSummarizer overallRatings_summarizer)
        {
            this.Initialize("", overallRatings_summarizer);
        }
        public Doable(string activityName, RatingSummarizer overallRatings_summarizer)
        {
        ...
        private void Initialize(string activityName, RatingSummarizer overallRatings_summarizer)
        {
            ...
            this.parents = new List<Doable>();
            this.parentsUsedForPrediction = new List<Doable>();
        ...
        public virtual void AddParent(Activity newParent)
        {
        ...
        public List<Doable> ParentsUsedForPrediction
        {
            get
            {
        ...
        public List<Doable> GetAllSuperactivities()
        {
            List<Doable> superCategories = new List<Doable>();
            superCategories.Add(this);
            int i = 0;
            for (i = 0; i < superCategories.Count; i++)
            {
                Doable Doable = superCategories[i];
                foreach (Doable parent in Doable.Parents)
                {
        ...
        public List<Doable> GetParticipationPredictionActivities()
        {
            List<Doable> activities = new List<Doable>();
            activities.Add(this);
            /*
        ...
        private void AddParticipationDatapoint(DateTime when, WillingnessSummary willingness)
        {
            ...
            List<Doable> activities = this.GetParticipationPredictionActivities();
            foreach (Doable Doable in activities)
            {
        ...
        public virtual List<Doable> GetChildren()
        {
            return new List<Doable>(0);
        }
        ...
        */
        ...
        private bool shouldUseNewlyAddedParentForPrediction(Doable parent)
        {
        ...
        private List<Doable> parents;
        private List<Doable> parentsUsedForPrediction;
